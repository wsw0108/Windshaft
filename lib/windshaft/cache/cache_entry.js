function CacheEntry(cacheBuster) {
    this.ready = false;
    this.err = null;
    this.renderer = null;
...
CacheEntry.prototype.getTile = function(z, res, xmin, ymin, cb) {
    this.renderer.getTile.apply(this.renderer, [z, res, xmin, ymin, cb]);
};

CacheEntry.prototype.getMetadata = function(cb) {
    this.renderer.getMetadata.apply(this.renderer, [cb]);
};
...
CacheEntry.prototype.setReady = function(err, renderer) {
    // consistency check
    if ( this.ready ) {
        throw new Error("Invalid call to CacheEntry.setReady on an ready entry");
    }
    this.ready = true;
    this.err = err;
...
CacheEntry.prototype._destroy = function() {
    ...
    // integrity checks
    if ( this.cb.length ) {
        throw new Error("CacheEntry._destroy was called while still having " + this.cb.length + " callbacks pending");
    }
    if ( this._refcount ) {
        throw new Error("CacheEntry._destroy was called while still having " + this._refcount + " references");
    }
